...
class Package:
    ...
    #   returnt he status as "ENROUTE"
    def __str__(self):
        # if "DELIVERED" not in self.status:
        #    return f"{self.id}, {self.status}ENROUTE, {self.address_name}, {self.delivery_address}, {self.deadline}, {self.delivery_city}, {self.delivery_zip}, {self.weight}"
        # else:
